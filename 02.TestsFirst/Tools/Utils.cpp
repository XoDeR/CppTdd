...
static string s_logFileName = "\\AmstelTechTestTools.log";
...
_tstring GetCurrentDirectory()
{
   ...
   unique_ptr<TCHAR> spBuf = std::make_unique<TCHAR[]>(size);
...
_tstring GetFileVersion()
{
   ...
   if (verSize != 0)
   {
	   unique_ptr<BYTE> spBuffer = std::make_unique<BYTE[]>(verSize);
      ...
      if (::GetFileVersionInfo(pModuleFileName, 0, verSize, spBuffer.get()))
      {
         ...
         if (::VerQueryValue(spBuffer.get(), 
              const_cast<LPTSTR>(_T("\\StringFileInfo\\01010110\\ProductVersion")), 
              (void**)&pVersion, 
              &verLen))
         {
            version = pVersion;
